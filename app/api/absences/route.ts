import { type NextRequest, NextResponse } from "next/server"
import { createAbsence, getAbsencesByStudentId } from "@/lib/database"
import { cookies } from "next/headers"

async function getUserFromSession() {
  const cookieStore = await cookies()
  const session = cookieStore.get("session")

  if (!session) return null

  try {
    const sessionData = JSON.parse(atob(session.value))
    if (sessionData.exp < Date.now()) return null
    return sessionData
  } catch {
    return null
  }
}

export async function GET(request: NextRequest) {
  try {
    const user = await getUserFromSession()
    if (!user) {
      return NextResponse.json({ error: "Non autorisé" }, { status: 401 })
    }

    const { searchParams } = new URL(request.url)
    const studentId = searchParams.get("studentId")

    if (!studentId) {
      return NextResponse.json({ error: "ID élève requis" }, { status: 400 })
    }

    const absences = await getAbsencesByStudentId(studentId)
    return NextResponse.json(absences)
  } catch (error) {
    console.error("Get absences error:", error)
    return NextResponse.json({ error: "Erreur lors de la récupération des absences" }, { status: 500 })
  }
}

export async function POST(request: NextRequest) {
  try {
    const user = await getUserFromSession()
    if (!user) {
      return NextResponse.json({ error: "Non autorisé" }, { status: 401 })
    }

    const absenceData = await request.json()
    absenceData.reported_by = user.userId

    const newAbsence = await createAbsence(absenceData)

    return NextResponse.json(newAbsence, { status: 201 })
  } catch (error) {
    console.error("Create absence error:", error)
    return NextResponse.json({ error: "Erreur lors de la création de l'absence" }, { status: 500 })
  }
}
